...
// UpsertDevicesAttributesWithUpdated provides a mock function with given fields: ctx, ids, attrs, scope, etag, lastUpdateDurationThreshold
func (_m *DataStore) UpsertDevicesAttributesWithUpdated(ctx context.Context, ids []model.DeviceID, attrs model.DeviceAttributes, scope string, etag string, lastUpdateDurationThreshold time.Duration) (*model.UpdateResult, error) {
	ret := _m.Called(ctx, ids, attrs, scope, etag, lastUpdateDurationThreshold)

	var r0 *model.UpdateResult
	if rf, ok := ret.Get(0).(func(context.Context, []model.DeviceID, model.DeviceAttributes, string, string, time.Duration) *model.UpdateResult); ok {
		r0 = rf(ctx, ids, attrs, scope, etag, lastUpdateDurationThreshold)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.UpdateResult)
		}
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, []model.DeviceID, model.DeviceAttributes, string, string, time.Duration) error); ok {
		r1 = rf(ctx, ids, attrs, scope, etag, lastUpdateDurationThreshold)
	} else {
		r1 = ret.Error(1)
	}
